...
public class TekstEventyr {
     ...
     *
     * @param tastatur Tastatur for innlesing av kommandoer.
    ...
    private static void brukFraSekk(Scanner tastatur, Spiller spiller, Rom rom) {
        ...
        System.out.printf("Kva vil du bruke?\nSekken innholder: ");
        System.out.printf(spiller.hentSekk().toString());

        String ting = tastatur.nextLine();
        // TODO Se etter ting i sekken, og om funnet, se om den kan brukes.
        if(!(ting.equals("bombe") || ting.equals("helsedrikk"))){
            System.out.println("Kan ikke brukes");
            return;
        }
        // TODO For helsedrikk, legg 1 til spillerens helse.
        if(ting.equals("helsedrikk") && spiller.hentSekk().hentTingIndeksFraNavn(ting)!=-1) {
            spiller.settHelse(spiller.hentHelse() + 1);
            spiller.hentSekk().fjernTing(spiller.hentSekk().hentTingIndeksFraNavn(ting));
        }
        // TODO For bombe, trekk 1 fra monsterets helse dersom det er et monster i rommet.
        if(ting.equals("bombe") && spiller.hentSekk().hentTingIndeksFraNavn(ting)!=-1) {
            if(rom.hentMonster()!=null)
                rom.hentMonster().settHelse(rom.hentMonster().hentHelse() - 1);
            spiller.hentSekk().fjernTing(spiller.hentSekk().hentTingIndeksFraNavn(ting));
        }
    }
    ...
    private static void taFraRom(Scanner tastatur, Spiller spiller, Rom rom, String taTekst) {
        ...
        if(rom.hentTingSamling().hentTingIndeksFraNavn(taTekst) == -1){
            return;
        }
        Ting ting = rom.hentTingSamling().hentTingFraNavn(taTekst);
        // TODO Ta tingen bort fra rommet.
        rom.hentTingSamling().fjernTing(rom.hentTingSamling().hentTingIndeksFraNavn(taTekst));
        // TODO Legg tingen i spillerens sekk.
        spiller.hentSekk().leggTilTing(ting);

        System.out.printf(taTekst + " er naa lagt til i sekken.\n" + spiller.hentNavn() + " har naa " + spiller.hentSekk().hentTingFraNavn(taTekst).hentAntall() + " " + taTekst+"\n");
    }
     ...
     */
    private static boolean lesBrettFraFil(String filBane) throws IOException {
        int romtall, startRoom;
        String string;
        String[] strings;
        // TODO Aapne fil for lesing.
        BufferedReader br = new BufferedReader(new FileReader(new File(filBane)));
        // TODO Les inn foerste linje: antall rom; indeks for start-rom.
        string = br.readLine();
        strings = string.split(" ");
        romtall = Integer.parseInt(strings[0]);
        startRoom = Integer.parseInt(strings[1]);
        // TODO Lag rom.
        Rom rom0 = new Rom();
        Rom rom1 = new Rom();
        Rom rom2 = new Rom();
        ...
        string = br.readLine();
        strings = string.split(" ");
        Rom.kobleVestOest(rom1,rom0);
        string = br.readLine();
        strings = string.split(" ");
        Rom.kobleNordSoer(rom1,rom2);
        string = br.readLine();
        strings = string.split(" ");
        Rom.kobleNordSoer(rom2, rom3);
        ...
        string = br.readLine();
        strings = string.split(" ");
        rom3.settMonster(new Monster(Integer.parseInt(strings[2]),Integer.parseInt(strings[3])));
